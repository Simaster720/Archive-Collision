...
- [ ] 프로덕션이 시트 데이터로 200개 표시. — **대기**: Vercel env(Production·Preview) 설정 후 develop push.
- [ ] 새로고침 버튼 → 1~2분 내 시트 수정분 반영(훅 URL 비노출). — **대기**: 배포 후 E2E.
- [x] 시트 fetch 실패 시 CI 빌드 실패. — 로컬 검증(키 누락 시 exit 1). 직전 배포 유지는 Vercel 네이티브 동작(배포 후 확인).
